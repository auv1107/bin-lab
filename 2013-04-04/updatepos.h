...
void update_my_plane(int press)
{
	switch(press)
	{
		...
		case K_DOWN:
			{
				my_current_plane->current_pos.y += my_current_plane->ver_speed;
				if (my_current_plane->current_pos.y > screen_floor_limit - my_current_plane->height / 2)
					my_current_plane->current_pos.y = screen_floor_limit - my_current_plane->height / 2;
				break;
			}
		case K_LEFT:
			{
		...
		case K_RIGHT:
			{
				my_current_plane->current_pos.x += my_current_plane->hor_speed;
				if (my_current_plane->current_pos.x > screen_right_limit - my_current_plane->width / 2)
					my_current_plane->current_pos.x = screen_right_limit - my_current_plane->width / 2;
				break;
			}
		case 'q':
			{
